...
constructor TDumper.Create(const AProcess: TProcessInformation; AImageBase, AOEP, AIAT: NativeUInt);
begin
  FProcess := AProcess;
  FOEP := AOEP;
  FIAT := AIAT;
  ...
  FForwards := TDictionary<Pointer, Pointer>.Create(32);
  FForwardsType2 := TDictionary<Pointer, Pointer>.Create(16);
...
begin
  ...
  a := Pointer(IAT);
  for i := 0 to IATSize div 4 - 1 do
  begin
    //Log(ltInfo, IntToHex(Cardinal(a), 8) + ' -> ' + IntToHex(Cardinal(a^), 8));
    ...
    for RM in Modules.Values do
      ...
      begin
        //if ExtractFileExt(RM.Name) = '.aes' then
        //  Break; // ahuehuehuehue
        ...
        if RM.ExportTbl.ContainsKey(a^) then
          ...
          Log(ltFatal, 'IAT ' + IntToHex(Cardinal(a), 8) + ' -> API ' + IntToHex(Cardinal(a^), 8) + ' not in export table of ' + RM.Name + ' (likely a bogus entry)');
  ...
  begin
    Descriptors[i].FirstThunk := (FIAT - FImageBase) + Cardinal(Addresses[DLLNames[i]][0]) - Cardinal(IAT);
